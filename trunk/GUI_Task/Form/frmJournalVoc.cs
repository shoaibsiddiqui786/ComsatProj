...
namespace GUI_Task
{
    ...
    public partial class frmJournalVoc : Form
    {
        ...
        private void frmJournalVoc_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space)
            {
                ...
                if (grdVoucher.Rows.Count > 0)
                {
                    ...
                    lblAcID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GCol1.acid].Value.ToString();
                    msk_AccountID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GCol1.acstrid].Value.ToString();
                    lblAccountName.Text = grdVoucher.Rows[fEditRow].Cells[(int)GCol1.actitle].Value.ToString();
                    txtNarration.Text = grdVoucher.Rows[fEditRow].Cells[(int)GCol1.Desc].Value.ToString();
                    txtDebit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GCol1.debit].Value.ToString();
                    txtCredit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GCol1.credit].Value.ToString();
                    btn_Add.Text = "&Update";
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            {
                grdVoucher.Rows[fEditRow].Cells[(int)GCol1.acid].Value = lblAcID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GCol1.acstrid].Value = msk_AccountID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GCol1.actitle].Value = lblAccountName.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GCol1.Desc].Value = txtNarration.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GCol1.debit].Value = txtDebit.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GCol1.credit].Value = txtCredit.Text;
                btn_Add.Text = "&Add";
                pnlVocTran.Visible = false;
                grdVoucher.Focus();
                //msk_AccountID.Focus();
        ...
        private void SumVoc()
        {
            ...
            {
                if (grdVoucher.Rows[i].Cells[(int)GCol1.debit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GCol1.debit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                ...
                if (grdVoucher.Rows[i].Cells[(int)GCol1.credit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GCol1.credit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
        ...
        private bool GridValid()
        {
            ...
            try
            {
                ...
                {
                    // Check GL Account ID
                    if (grdVoucher.Rows[lLastRow].Cells[(int)GCol1.acstrid].Value == null) // || 
                    {
                        rtnValue = false;
                    }
                    else
                    {
                        if ((grdVoucher.Rows[lLastRow].Cells[(int)GCol1.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            rtnValue = false;
                        }
                    }
        ...
        private bool PrepareDocDetail()
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GCol1.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GCol1.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    //
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GCol1.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                    //                                                                                       // 5- Ac Title NA
                    lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GCol1.Desc].Value.ToString()) + "'";      // 8- Narration 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GCol1.debit].Value.ToString();           // 9- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GCol1.credit].Value.ToString();          // 10- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GCol1.snum].Value.ToString();          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GCol1.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GCol1.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
...
//where doc_vt_id=1 and doc_fiscal_id=1
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        ...
                        grdVoucher.Rows.Add(
                            dRow.ItemArray.GetValue((int)GCol1.Status).ToString(),                       // 0-
                            dRow.ItemArray.GetValue((int)GCol1.tranid).ToString(),                           // 1-
                            dRow.ItemArray.GetValue((int)GCol1.snum).ToString(),                           // 1-
                            dRow.ItemArray.GetValue((int)GCol1.acid).ToString(),                             // 3-
                            dRow.ItemArray.GetValue((int)GCol1.acstrid).ToString(),                          // 4-
                            dRow.ItemArray.GetValue((int)GCol1.actitle).ToString(),                          // 5-
                            dRow.ItemArray.GetValue((int)GCol1.Desc).ToString(),                            // 6-
                            dRow.ItemArray.GetValue((int)GCol1.debit).ToString(),                            // 9-
                            dRow.ItemArray.GetValue((int)GCol1.credit).ToString()                           // 10-
                            );
        ...
        private void btn_PinIDCr_Click(object sender, EventArgs e)
        {
            ...
            {
                btn_PinIDCr.Text = "&Un Pin";
               // btn_PinIDCr.Image = GUI_Task.Properties.Resources.tiny_pinned;
                txtCredit.Enabled = false;
                btn_PinIDDr.Text = "&Pin";
                txtDebit.Enabled = true;
                txtNarration.Text = "PAYMENT ";
