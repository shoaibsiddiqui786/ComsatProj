...
namespace GUI_Task
{
    public partial class frmInvoiceEntryCF : Form
    {
       ...
       // bool blnFormLoad = true;
        ...
        private void frmInvoiceEntryCF_Load(object sender, EventArgs e)
        {
            AtFormLoad();
           // blnFormLoad = false;
        ...
        private void frmInvoiceEntryCF_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
                this.Close();
        }

        private void txtCashInvNo_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void txtCashInvNo_DoubleClick(object sender, EventArgs e)
        {
            LookUp_Voc();
        }
        ...
        private void LookUp_Voc()
        {
            ...
            frmLookUp sForm = new frmLookUp(
                    "d.Inv_No",
                " d.Inv_Date, h.Name, d.Qty, d.Amount ",
                " Inv_CashDet d inner join Heads h on d.Customer = h.Code ",
                    this.Text.ToString(),
                    1,
                    "Invoice No ,Date ,Customer Name, Qty, Amount",
                    "12,8,15,8,10",
                    " T, T, T, T, T",
                    true,
                    "",
            ...
            txtCashInvNo.Text = string.Empty;
            sForm.lupassControl = new frmLookUp.LUPassControl(PassDataVocID);
            sForm.ShowDialog();

            if (txtCashInvNo.Text != null)
            {
                if (txtCashInvNo.Text != null)
                {
                    if (txtCashInvNo.Text.ToString() == "" || txtCashInvNo.Text.ToString() == string.Empty)
                    {
                        return;
                    }
                    if (txtCashInvNo.Text.ToString().Trim().Length > 0)
                    {
                        PopulateRecords();
                        //LoadSampleData();
                        //SumVoc();
        ...
        private void PassDataVocID(object sender)
        {
            ...
            txtCashInvNo.Text = ((TextBox)sender).Text;
        ...
        private void PopulateRecords()
        {
            ...
            tSQL = "select d.Inv_No, d.Inv_Date, d.SaleType, d.Customer, h.Name AS CustomerName, d.AddaID, d.Qty, d.Amount ";
            tSQL += " from Inv_CashDet d ";
            tSQL += " inner join Heads h ON d.Customer = h.Code  ";
            tSQL += " where d.Inv_No='" + txtCashInvNo.Text.ToString() + "';";

            try
            {
                ds = clsDbManager.GetData_Set(tSQL, "Inv_CashDet");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    //fAlreadyExists = true;
                    dRow = ds.Tables[0].Rows[0];
                    // Starting title as 0
                    txtCashInvNo.Text = (ds.Tables[0].Rows[0]["Inv_No"] == DBNull.Value ? "" : ds.Tables[0].Rows[0]["Inv_No"].ToString());
                    ...
                    dtpDate.Value =  Convert.ToDateTime(ds.Tables[0].Rows[0]["Inv_Date"]);
