...
namespace GUI_Task
{
    ...
    public partial class frmPackingFormulaDefinition : Form
    {
        ...
        public frmPackingFormulaDefinition()
        {
            InitializeComponent();
        }

        private void frmPackingFormulaDefinition_Load(object sender, EventArgs e)
        {
            AtFormLoad();
            blnFormLoad = false;
        ...
        private void frmPackingFormulaDefinition_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void LookUp_Voc()
        {
            frmLookUp sForm = new frmLookUp(
                    ...
                    "i.ItemCode, i.Name AS ItemName, sz.cgdDesc AS SizeName, clr.cgdDesc AS ColorName, p.Date, pd.Qty, p.Note, CASE [Status] WHEN 0 THEN 'InActive' WHEN 1 THEN 'ACTIVE' END AS [Status] ",
                     " PackFormula p  INNER JOIN Item i ON p.ItemId = i.ItemId INNER JOIN CatDtl sz ON sz.cgdCode = p.SizeID AND sz.cgCode = 5 "
                    + " INNER JOIN CatDtl clr ON clr.cgdCode = p.ColorID AND clr.cgCode = 3 INNER JOIN PackFormulaDetail pd ON pd.ItemId = p.ItemId ",
                    this.Text.ToString(),
                    1,
        ...
        private void PopulateRecords()
        {
            ...
            tSQL = " select p.PackFormulaId, p.ItemId, i.ItemCode, i.Name AS ItemName, sz.cgdDesc AS SizeName, ";
            tSQL += " clr.cgdDesc AS ColorName, p.Date, pd.Qty, p.Note, ";
            tSQL += " CASE [Status] WHEN 0 THEN 'InActive' WHEN 1 THEN 'ACTIVE' END AS [Status] ";
            tSQL += " from PackFormula p INNER JOIN Item i ON p.ItemId = i.ItemId ";
        ...
        private void LoadGridData()
        {
            ...
            lSQL += " from PackFormulaDetail pd ";
            ...
            lSQL += " where pd.PackFormulaId ='" + txtFormula.Text.ToString() + "'; ";
       ...
       private void SumVoc()
        {
            ...
            {
                if (grd.Rows[i].Cells[(int)GColPFD.Qty].Value != null)
                {
                    bcheck = decimal.TryParse(grd.Rows[i].Cells[(int)GColPFD.Qty].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
        ...
        private bool PrepareDocMaster()
        {
            ...
            try
            {
                if (txtFormula.Text.ToString().Trim(' ', '-') == "")
                {
                    ...
                    fDocID = clsDbManager.GetNextValDocID("PackFormula", "PackFormulaId", fDocWhere, "");

                    lSQL = "insert into PackFormula (";
                    lSQL += "  PackFormulaId ";                              //  0-    ItemID";   
                    lSQL += ", Date ";                                //  1-    ItemCod  
                ...
                {
                    fDocWhere = " PackFormulaId = '" + txtFormula.Text.ToString() + "'";
                    if (clsDbManager.IDAlreadyExistWw("PackFormula", "PackFormulaId", fDocWhere))
                    {

                        fDocAlreadyExists = true;
                        lSQL = "delete from PackFormulaDetail ";
        ...
        private bool PrepareDocDetail()
        {
            ...
            try
            {
                for (int dGVRow = 0; dGVRow < grd.Rows.Count; dGVRow++)
                {
                    ...
                    lSQL = "INSERT INTO PackFormulaDetail (PackFormulaId";
                    lSQL += ",ItemId,SizeId,ColorID,GodownId,Qty)";
                    lSQL += " VALUES (";
                    lSQL += "'" + txtFormula.Text.ToString() + "'";
                    lSQL += ", " + grd.Rows[dGVRow].Cells[(int)GColPFD.ItemID].Value.ToString() + "";
                    lSQL += ", " + grd.Rows[dGVRow].Cells[(int)GColPFD.SizeID].Value.ToString() + "";
                    lSQL += ", " + grd.Rows[dGVRow].Cells[(int)GColPFD.ColorID].Value.ToString() + "";
                    lSQL += ", " + grd.Rows[dGVRow].Cells[(int)GColPFD.GodownID].Value.ToString() + "";
                    lSQL += ", " + grd.Rows[dGVRow].Cells[(int)GColPFD.Qty].Value.ToString() + "";
                    lSQL += ")";
                    fManySQL.Add(lSQL);
                } // End For loopo
                return rtnValue;
